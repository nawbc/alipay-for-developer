...
function veriySign(params) {
  try {
    ...
    var key = typeof (privatePem) == "string" ? privatePem : privatePem.toString();
...
exports.initClient = function (params) {
  ...
  var isPemFile = /.*\.pem$/;
  try {
    private_key = isPemFile.test(params.private_key) ? fs.readFileSync(params.private_key) : params.private_key;
  } catch (err) {
    console.log(err);
    return;
  }
